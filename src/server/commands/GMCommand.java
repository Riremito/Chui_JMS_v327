...
public class GMCommand {
    ...
    public static class 五轉 extends CommandExecute {
    ...
    public static class 封印道具 extends CommandExecute {
        ...
        public int execute(MapleClient c, String[] splitted) {
            ...
            for (Item item : chr.getInventory(type).listById(itemid)) {
                ...
                chr.getClient().announce(InventoryPacket.updateSpecialItemUse(item, type.getType(), item.getPosition(), true, chr));
            }
            if (type == MapleInventoryType.EQUIP) {
                type = MapleInventoryType.EQUIPPED;
                for (Item item : chr.getInventory(type).listById(itemid)) {
                    item.setFlag((byte) (item.getFlag() | ItemFlag.LOCK.getValue()));
                    //chr.getClient().announce(CField.updateSpecialItemUse(item, type.getType()));
                }
            }
