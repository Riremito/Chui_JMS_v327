...
public class Event_PyramidSubway {
    ...
    public final void commenceTimerNextMap(final MapleCharacter c, final int stage) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null) {
                    chr.getClient().announce(CField.getClock(time));
                    chr.getClient().announce(CField.showEffect("killing/first/number/" + stage));
                    chr.getClient().announce(CField.showEffect("killing/first/stage"));
                    chr.getClient().announce(CField.showEffect("killing/first/start"));
                    fullUpdate(chr, stage);
                }
            }
        } else {
            c.getClient().announce(CField.getClock(time));
            c.getClient().announce(CField.showEffect("killing/first/number/" + stage));
            c.getClient().announce(CField.showEffect("killing/first/stage"));
            c.getClient().announce(CField.showEffect("killing/first/start"));
            fullUpdate(c, stage);
        }
    ...
    public final void succeed(final MapleCharacter c) {
        ...
        c.getClient().announce(CField.showEffect("killing/clear"));
        c.getClient().announce(CField.sendPyramidResult(rank, exp));
        dispose(c);
    }
    ...
    public final void broadcastUpdate(final MapleCharacter c) {
        c.getClient().announce(CField.sendPyramidUpdate(bar));
    }

    public final void broadcastEffect(final MapleCharacter c, final String effect) {
        c.getClient().announce(CField.showEffect(effect));
    }

    public final void broadcastEnergy(final MapleCharacter c, final String type, final int amount) {
        c.getClient().announce(CWvsContext.sendPyramidEnergy(type, String.valueOf(amount)));
    }
    ...
    private static void changeMap(final MapleCharacter c, final MapleMap map, final int minLevel, final int maxLevel, final int clear) {
        ...
        if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
            for (MaplePartyCharacter mpc : c.getParty().getMembers()) {
                ...
                if (chr != null && chr.getId() != c.getId() && chr.getLevel() >= minLevel && chr.getLevel() <= maxLevel) {
                    if (clear == 1) {
                        chr.getClient().announce(CField.showEffect("killing/clear"));
                    } else if (clear == 2) {
                        chr.getClient().announce(CField.showEffect("killing/fail"));
                    }
        ...
        if (clear == 1) {
            ...
            c.getClient().announce(CField.showEffect("killing/fail"));
        }
