...
public class MapleInventoryManipulator {
    ...
    public static void addRing(MapleCharacter chr, int itemId, int ringId, int sn, String partner) {
        ...
        chr.getClient().announce(CSPacket.sendBoughtRings(ItemConstants.類型.戀人戒指(itemId), ring, sn, chr.getClient().getAccID(), partner));
    }
    ...
    public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period, int periodMod, String gmLog) {
        ...
        if (itemId >= 4031332 && itemId <= 4031341) {
            c.announce(CField.getGameMessage(8, "Hint: Use @event to exchange a certificate of straight wins."));
        }
...
/*                if (isMonsterDrop && (eq.getState(false) > 0 || eq.getState(true) > 0)) {
                    c.announce(CField.EffectPacket.showItemTopMsgEffect(item));
                }*/
                c.announce(InventoryPacket.addInventorySlot(type, item, true));
                if (GameConstants.isHarvesting(item.getItemId())) {
                    c.getPlayer().getStat().handleProfessionTool(c.getPlayer());
    ...
    public static boolean removeFromSlot(final MapleClient c, final MapleInventoryType type, final short slot, short quantity, final boolean fromDrop, final boolean consume) {
        ...
        if (item != null) {
            ...
            if (item.getQuantity() == 0 && !allowZero) {
                ...
                c.announce(InventoryPacket.updateInventorySlot(type, item, fromDrop));
            }
            return true;
        }
        return false;
    ...
    public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP) && initialTarget != null
            ...
            if ((olddstQ + oldsrcQ) > slotMax) {
                ...
                c.announce(InventoryPacket.moveAndMergeInventoryItem(type, src, dst, c.getPlayer().getInventory(type).getItem(dst).getQuantity(), bag, switchSrcDst, bothBag));
            }
        } else {
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (source.getItemId() / 10000 == 166) {
            if (source.getAndroid() == null) {
                ...
                source.setFlag(flag);
                c.announce(CWvsContext.InventoryPacket.updateSpecialItemUse_(source, MapleInventoryType.EQUIP.getType(), c.getPlayer()));
            }
...
//                    StructItemOption pot = ii.getPotentialInfo(i).get(ii.getReqLevel(source.getItemId()) / 10);
        ...
        c.announce(InventoryPacket.moveInventoryItem(MapleInventoryType.EQUIP, src, takeOff == 0 ? dst : takeOff, (byte) 2, false, false));
        if (takeOff != 0) {
            c.announce(InventoryPacket.moveInventoryItem(MapleInventoryType.EQUIP, dst, src, (byte) 2, false, false));
        }
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        if (ItemConstants.類型.武器(source.getItemId())) {
            ...
            c.getPlayer().removeAndroid();
            c.announce(CField.removeAndroidHeart());
        } else if (source.getItemId() == 1122017) {
