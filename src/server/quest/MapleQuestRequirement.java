...
public class MapleQuestRequirement implements Serializable {
    ...
    public boolean check(MapleCharacter c, Integer npcid) {
        switch (type) {
            ...
            case fieldEnter:
                if (intStore > 0) {
                    if (quest.getId() > 1400 && quest.getId() < 1406) {//獲取WZ有問題,多個地圖的WZ無法獲取後面的
                        switch (c.getMapId()) {
                            case 102000003:
                            case 101000003:
                            case 100000201:
            ...
            case subJobFlags: // 1 for non-DB, 2 for DB...
                return c.getSubcategory() == (intStore / 2) || (quest.getId() == 32240 && c.getJob()/100 > 0 && c.getJob()/100 < 6);//判斷有點問題...
            case craftMin:
            case willMin:
            case charismaMin:
            case insightMin:
