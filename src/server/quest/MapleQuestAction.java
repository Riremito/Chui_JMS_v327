    ...
    public void runStart(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
                    ...
                    if (canGetItem(item, c)) {
                        ...
                        if ((item.prop == -2)
                            ...
                            if (count < 0) {
                                ...
                                c.getClient().announce(CWvsContext.InfoPacket.getShowItemGain(id, count, true));
                            } else {
                        ...
                        MapleInventoryManipulator.addById(c.getClient(), id, count, "", null, period, MapleInventoryManipulator.DAY, "從任務 " + toString() + " 中獲得, 時間:" + FileoutputUtil.CurrentReadable_Date());
                        c.getClient().announce(InfoPacket.getShowItemGain(id, count, true));
                            }
                        }
                    }
                }
            ...
            case nextQuest:
                status = c.getQuest(this.quest);
                if (status.getForfeited() <= 0) {
                    c.getClient().announce(CField.updateQuestFinish(this.quest.getId(), status.getNpc(), this.intStore));
                }
                break;
            case money:
                status = c.getQuest(this.quest);
            ...
            case pop:
                ...
                if (status.getForfeited() <= 0) {
                    ...
                    c.getClient().announce(CWvsContext.InfoPacket.getShowFameGain(fameGain));
                }
                break;
            case buffItemID:
                status = c.getQuest(this.quest);
                ...
                if (status.getForfeited() <= 0) {
                    ...
                    if (this.applicableJobs.size() > 0) {
                        ...
                        if (finalJob == 0) {
                            ...
                            c.gainSP(sp_val, GameConstants.getSkillBook(finalJob, c.getLevel()));
                        }
                    } else {
                        c.gainSP(sp_val);
                    }
    ...
    public void runEnd(MapleCharacter c, Integer extSelection) {
        ...
        switch (type) {
            ...
            case item:
                ...
                break;
            case nextQuest:
                c.getClient().announce(CField.updateQuestFinish(this.quest.getId(), c.getQuest(this.quest).getNpc(), this.intStore));
                break;
            case money:
                c.gainMeso(this.intStore, true, true);
                break;
