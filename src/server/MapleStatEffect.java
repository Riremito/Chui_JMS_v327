...
//                        break;
    ...
    public void applySummonEffect(MapleCharacter player, long time, boolean bl2) {
        ...
        if (cooldown > 0) {
            int n3 = GameConstants.getLinkedAttackSkill(sourceid);
/*            if (sourceid == 主教.神圣保护 || sourceid == 夜行者.黑暗重生 || sourceid == 幻影.神秘的运气) {
            ...
            if (sourceid == 机械师.磁场) {
                ...
                player.getMap().broadcastMessage(CWvsContext.teslaTriangle(player.getId(), arrayList.get(0), arrayList.get(1), arrayList.get(2)));
            }*/
            if (!player.skillisCooling(n3 == 1320016 ? sourceid : n3)) {
                player.addCooldown(n3 == 1320016 ? sourceid : n3, time, cooldown * 1000);
            }
        }
    }
    ...
    public void applyQuiverKartrige(final MapleCharacter applyfrom) {
        int mode = 0;
        if (applyfrom.getBuffedValue(MapleBuffStat.QUIVER_KARTRIGE) != null) {
            mode = applyfrom.getBuffedValue(MapleBuffStat.QUIVER_KARTRIGE);
        }
...
/*                    if (mountid != 0 && mountid2 != 0) {
    ...
    public boolean is狂龍變形() {
        return (this.skill) && ((this.sourceid == 61111008) || (this.sourceid == 61120008) || (this.sourceid == 61121053));
    }
}
