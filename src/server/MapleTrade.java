...
public class MapleTrade {
    ...
    public final void CompleteTrade() {
        ...
        this.chr.get().getClient().announce(CField.InteractionPacket.TradeMessage(this.tradingslot, (byte) 7));
    }
    ...
    public final void cancel(MapleClient c, MapleCharacter chr, int unsuccessful) {
        ...
        c.announce(CField.InteractionPacket.getTradeCancel(this.tradingslot, unsuccessful));
    }
    ...
    public final void setMeso(int meso) {
        ...
        if (this.chr.get().getMeso() >= meso) {
            ...
            this.meso += meso;
            this.chr.get().getClient().announce(CField.InteractionPacket.getTradeMesoSet((byte) 0, this.meso));
            if (this.partner != null) {
                this.partner.getChr().getClient().announce(CField.InteractionPacket.getTradeMesoSet((byte) 1, this.meso));
            }
        }
    }
    ...
    public final void addItem(Item item) {
        ...
        this.items.add(item);
        this.chr.get().getClient().announce(CField.InteractionPacket.getTradeItemAdd((byte) 0, item));
        if (this.partner != null) {
            this.partner.getChr().getClient().announce(CField.InteractionPacket.getTradeItemAdd((byte) 1, item));
        }
    }
    ...
    public final void chat(String message) throws Exception {
        if (!CommandProcessor.processCommand(chr.get().getClient(), message, CommandType.TRADE)) {
            ...
            if (this.partner != null) {
                this.partner.getChr().getClient().announce(PlayerShopPacket.shopChat(this.chr.get().getName() + " : " + message, 1));
            }
        }
    ...
    public final void chatAuto(String message) {
        ...
        if (this.partner != null) {
            this.partner.getChr().getClient().announce(PlayerShopPacket.shopChat(message, 1));
        }
        if (this.chr.get().getClient().isMonitored()) {
    ...
    public final boolean setItems(MapleClient c, Item item, byte targetSlot, int quantity) {
        ...
        if ((this.partner == null) || (target == -1) || (ItemConstants.類型.寵物(item.getItemId())) || (isLocked()) || ((GameConstants.getInventoryType(item.getItemId()) == MapleInventoryType.EQUIP) && (quantity != 1))) {
            return false;
        }
        short flag = item.getFlag();
    ...
    public static final void completeTrade(MapleCharacter c) {
        ...
        local.locked = true;
        partner.getChr().getClient().announce(CField.InteractionPacket.getTradeConfirmation());
    ...
    public static final void startTrade(MapleCharacter c) {
        if (c.getTrade() == null) {
            ...
            c.getClient().announce(CField.InteractionPacket.getTradeStart(c.getClient(), c.getTrade(), (byte) 0));
        } else {
            c.getClient().announce(CWvsContext.broadcastMsg(5, "You are already in a trade"));
        }
    }
    ...
    public static final void inviteTrade(MapleCharacter c1, MapleCharacter c2) {
        ...
        if ((c2 != null) && (c2.getTrade() == null)) {
            ...
            c2.getClient().announce(CField.InteractionPacket.getTradeInvite(c1));
        } else {
            c1.getClient().announce(CWvsContext.broadcastMsg(5, "The other player is already trading with someone else."));
    ...
    public static final void visitTrade(MapleCharacter c1, MapleCharacter c2) {
        if ((c2 != null) && (c1.getTrade() != null) && (c1.getTrade().getPartner() == c2.getTrade()) && (c2.getTrade() != null) && (c2.getTrade().getPartner() == c1.getTrade())) {
            ...
            c1.getClient().announce(CField.InteractionPacket.getTradeStart(c1.getClient(), c1.getTrade(), (byte) 1));
            ...
            c1.getClient().announce(CWvsContext.broadcastMsg(5, "The other player has already closed the trade"));
        }
    }
