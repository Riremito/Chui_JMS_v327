...
public class MapleCoconut extends MapleEvent {
    ...
    public void onMapLoad(MapleCharacter chr) {
        super.onMapLoad(chr);
        chr.getClient().announce(CField.coconutScore(getCoconutScore()));
    }
                ...
                if (getMapleScore() == getStoryScore()) {
                    ...
                    for (MapleCharacter chr : getMap(0).getCharactersThreadsafe()) {
                        if (chr.getTeam() == (getMapleScore() > getStoryScore() ? 0 : 1)) {
                            chr.getClient().announce(CField.showEffect("event/coconut/victory"));
                            chr.getClient().announce(CField.playSound("Coconut/Victory"));
                        } else {
                            chr.getClient().announce(CField.showEffect("event/coconut/lose"));
                            chr.getClient().announce(CField.playSound("Coconut/Failed"));
                        }
                    }
                    warpOut();
                }
