...
//            mplew.write(0);
    ...
    public static void addInventoryInfo(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        for (Item item : equipped) {
            if ((item.getPosition() <= -1000) && (item.getPosition() > -1100)) {//龍魔導寶貝龍裝備
                addItemPosition(mplew, item, false, false);
                addItemInfo(mplew, item, chr);
            }
        }
        ...
        for (Item item : equipped) {
            if ((item.getPosition() <= -1700) && (item.getPosition() > -1800)) {// 技能造型
                addItemPosition(mplew, item, false, false);
                addItemInfo(mplew, item, chr);
            }
        }
...
//        mplew.writeInt(0);
    ...
    public static void addCharStats(MaplePacketLittleEndianWriter mplew, MapleCharacter chr) {
        ...
        mplew.write(5/*6*/); // 日版是5 台版是6 用台版的mask凱撒1~4轉會終止遊戲
        mplew.write(5/*7*/); // 日版是5 台版是7 用台版的mask凱撒1~4轉會終止遊戲
        mplew.writeInt(0);
...
/*        mplew.writeShort(equip.getFusionAnvil() % 10000);
        ...
        mplew.writeInt(-1);  // this is nPrevBonusExpRate

        // GW_CashItemOption::Decode 開始
        mplew.writeLong(0);
        mplew.writeLong(getTime(-2));
        mplew.writeInt(0);
        for (int i = 0; i < 3; i++) {
        ...
        // GW_CashItemOption::Decode 結束

        // 魂武
        mplew.writeInt(0/*equip.getSoulName()*/);
        mplew.write(0/*equip.getSoulPotential() % 100*/);
        mplew.write(0); // socket 1 max = 1000 3 max = 5000
        mplew.writeInt(0/*equip.getSoulEnchanter()*/);
...
//            mplew.writeInt(0);
        ...
        if ((mask & 2) != 0) {
            ...
            mplew.writeInt(0/*chr.getCSPoints(2)*/); // 楓葉點數
            mplew.writeLong(0);
            mplew.write(0);
            mplew.writeInt(0);
        }
        ...
        if ((mask & 0x1000) != 0) { // 獸魔激鬥
            int j = 0;
            mplew.writeInt(j);
            for (int i = 0; i < j; i++) {
                MonsterBattle_MobInfo(mplew);
            }
            ...
            if (v24) {
                MonsterBattleLadder_UserInfo(mplew);
            }
            int v25 = 0;
            mplew.write(v25);
            if (v25 > 0) {
                for (int i = 0; i < v25; i++) {
                    MonsterBattleRankInfo(mplew);
                }
            }
            int v26 = 0;
            mplew.write(v26);
        ...
        int v96 = 0;
        mplew.writeShort(v96); // 傳授技能
        for (int i = 0; i < v96; i++) {
            mplew.writeInt(0); // 傳授技能ID
            mplew.writeShort(0); // 傳授技能等級
        }
/*        int v93 = 0;
        mplew.writeInt(v93); // 傳授技能
        for (int i = 0; i < v93; i++) {
            mplew.writeInt(0);
            mplew.writeInt(0);
            mplew.writeInt(0); // 傳授技能ID
        ...
        if ((mask & 0x20000) != 0 && (mask & 0x10000) != 0 && (mask & 0x200000) != 0) {
        ...
        if ((mask & 0x4000000) != 0) {
            for (int i = 0; i < 10; i++) {
                mplew.write(0); // 如果 > 0 需要 additeminfo
            }
        }
        if ((mask & 0x800000) != 0) { // familar
            mplew.writeInt(0);
        }
        if ((mask & 0x8000) != 0) {
            mplew.writeInt(0);
...
/*        boolean v145 = false;
        ...
        if ((mask & 0x4000000) != 0) { // NpcShopBuyLimit
            int v144 = 0;
            mplew.writeShort(v144);//未知
            for (int i = 0; i < v144; i++) {
                mplew.writeShort(0);
                mplew.writeInt(0);
            }
        }
        if ((mask & 0x10000000) != 0 || (mask & 0x20000000) != 0) {
            addStealSkills(mplew, chr);//幻影複製技能訊息 [60+20]Byte
        }
...
/*        if ((mask & 0x20000) != 0) { // RunnerGameRecord
            mplew.writeInt(chr.getId());
            mplew.writeInt(0);
            mplew.writeInt(0);
            mplew.writeInt(0);
...
/*        // TextEquipInfo
        int TextEquipInfoSize = 0;
        mplew.writeInt(TextEquipInfoSize);
        for (int i = 0; i < TextEquipInfoSize; i++) {
            mplew.writeInt(0);
            mplew.writeMapleAsciiString("");
        }*/
        ...
        if ((mask & 0x8000) != 0) {
            int v123 = 0;
            mplew.writeShort(v123);
            for (int i = 0; i < v123; i++) {
                mplew.writeShort(0);
                mplew.writeShort(0);
            }
        }
...
/*        if (mask < 0) {
    ...
    public static void MonsterBattle_MobInfo(final MaplePacketLittleEndianWriter mplew) {
        mplew.writeInt(0);
        mplew.writeInt(0);
        mplew.writeInt(0);
        mplew.writeInt(0);
    ...
    public static void MonsterBattleLadder_UserInfo(final MaplePacketLittleEndianWriter mplew) {
        mplew.writeInt(0);
        mplew.writeInt(0);
        mplew.writeInt(0);
        mplew.writeInt(0);
    ...
    public static void MonsterBattleRankInfo(final MaplePacketLittleEndianWriter mplew) {
        mplew.write(0);
        mplew.writeInt(0);
        mplew.write(0);
        mplew.writeInt(0);
