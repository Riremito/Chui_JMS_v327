...
public class JobPacket {
    ...
    public static class AngelicPacket {
        ...
        public static byte[] DressUpTime(byte type) {
            ...
            mplew.write(1/*2*/);
            mplew.writeLong(PacketHelper.getTime(System.currentTimeMillis()) + 360000);
            return mplew.getPacket();
        }
        ...
        public static byte[] ChangeAngelic(int type) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            mplew.writeShort(SendPacketOpcode.CHANGE_ANGELIC.getValue());
            mplew.write(type);

            return mplew.getPacket();
        }

        public static byte[] DressUpInfo(int skillid) {
            MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
            mplew.writeShort(SendPacketOpcode.DRESSUP_INFO.getValue());
...
/*        public static byte[] updateLuminousGauge(int gauge, int type) {
        ...
        public static byte[] updateLuminousGauge(MapleCharacter chr) {
            ...
            mplew.writeShort(SendPacketOpcode.LUMINOUS_COMBO.getValue());
            mplew.writeInt(chr.getDarkGauge());
            mplew.writeInt(chr.getLightGauge());
            mplew.writeInt(chr.getDarkWing());
            mplew.writeInt(chr.getLightWing());
            mplew.writeInt((int) System.currentTimeMillis());//283183599 (according to old packet)
