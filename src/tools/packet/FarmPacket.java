...
public class FarmPacket {
    ...
    public static byte[] updateUserFarmInfo(MapleCharacter chr, boolean update) {
        ...
        if (update) {
            ...
            mplew.writeMapleAsciiString(WorldConstants.getById(chr.getWorld()).name());
    ...
    public static byte[] updateAvatar(Pair<WorldConstants.Option, MapleCharacter> from, Pair<WorldConstants.Option, MapleCharacter> to, boolean change) {
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.FARM_AVATAR.getValue());
        mplew.write(change);
        mplew.writeInt(from.getLeft().getWorld());
        mplew.writeMapleAsciiString(WorldConstants.getById(from.getLeft().getWorld()).name());
