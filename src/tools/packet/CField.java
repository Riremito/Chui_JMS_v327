...
//            UnkFunction(mplew); // [int]
        ...
        if (v87) {
            ...
            mplew.writeMapleAsciiString(""/*to.getCustomMapBgm()*/);
            mplew.writeInt(0/*to.getCustomBgMapID()*/);
        }
        mplew.writeBoolean(false);
...
//        mplew.writeZeroBytes(8);
    ...
    public static byte[] showEffect(String effect) {
        return environmentChange(effect, 12/*0x0D*/);//was 3
    }
...
//        mplew.write(active ? 0 : 1);
    ...
    public static byte[] gainForce(boolean isRemote, MapleCharacter chr, List<Integer> oid, int type, int skillid, List<Pair<Integer, Integer>> forceInfo) {
        ...
        if (type > 0 && type != 9/*!(type == 0 || type == 9 || type == 14)*/) {
            mplew.write(1);
...
/*        if (type == 16) {
    ...
    public static byte[] spawnPlayerMapobject(MapleCharacter chr) {
        ...
        mplew.writeInt(chr.getId());
        mplew.write(chr.getLevel()/*0*/);
        mplew.write(250/*0*/);// 等級上限
        ...
        mplew.writeInt(0);
        mplew.writeInt(0);
        mplew.writeInt(0/*chr.getDamageSkin()*/);
        mplew.writeInt(0);
        mplew.writeInt(0);
        mplew.writeInt(0);
        boolean v12 = false;
...
/*        int v65 = 0;
        ...
        mplew.writeInt(chr.getMount().getItemId());//騎寵id
...
/*        for (int i = 0; i < 5; i++) { // 不發送這5個-1就會看到人物頭上稱號一大堆
    ...
    public static void addAttackBody(MaplePacketLittleEndianWriter mplew, SendPacketOpcode op, MapleCharacter chr, int skilllevel, int itemId, AttackInfo attackInfo, boolean hasMoonBuff) {
        ...
        if (MapleJob.is神之子(attackInfo.skill / 10000) && attackInfo.skill != 100001283) {
            short zero1 = 0;
            short zero2 = 0;
...
/*        if (op == SendPacketOpcode.RANGED_ATTACK) {
            if (attackInfo.skill != 13121052) {
                mplew.write(0);
            }
            mplew.writeInt(8);
        } else {
            mplew.writeInt(GameConstants.Attacktype(attackInfo.skill));
        }*/
        mplew.write(0);
...
//                        }
        ...
        if (attackInfo.skill == 2321001 || attackInfo.skill == 2221052 || attackInfo.skill == 11121052) {
            mplew.writeInt(0);
        } else if (attackInfo.skill == 4221052
                || attackInfo.skill == 65121052
                || attackInfo.skill == 13121052
                || (attackInfo.skill - 13121052) == 1000000
                || (attackInfo.skill - 13121052) == 2000000
                || (attackInfo.skill - 13121052) == 66880377
                || (attackInfo.skill - 13121052) == 66880379
                || (attackInfo.skill - 13121052 - 66880379) == 36
                || attackInfo.skill == 101000202
                || attackInfo.skill == 101000102) {
            mplew.writeInt(0);
            mplew.writeInt(0);
        }
        if (attackInfo.skill == 13111020 || attackInfo.skill == 112111016) {
            mplew.writePos(chr.getPosition()); // Position
        }
        if (attackInfo.skill == 112110003) {
            mplew.writeInt(0);
        }
        ...
        if (op == SendPacketOpcode.RANGED_ATTACK) {
            if (MapleJob.is神之子(attackInfo.skill / 10000) || attackInfo.skill == 13121052) {
...
//        mplew.writeInt(0);//119
    ...
    public static byte[] updateCharLook(MapleCharacter chr, boolean second) {
        ...
        mplew.writeInt(chr.getId());
        int unk = /*MapleJob.is神之子(chr.getJob()) ? 9 : */1;
        mplew.write(unk);
        if ((unk & 1) != 0) {
...
//            mplew.writeInt(cid);
    ...
    public static byte[] spawnObtacleBomb(List<ObtacleAtom> bombs){
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.SPAWN_OBTACLE.getValue());        
    ...
    public static byte[] ClearObtacleBomb(List<ObtacleAtom> bombs){
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
    ...
    public static byte[] DebuffObjOn(int x, int y){
        MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();

        mplew.writeShort(SendPacketOpcode.DEBUFF_OBJ_ON.getValue());     
        mplew.write(1);
        mplew.writeInt(x);
        mplew.writeInt(y);
        mplew.writeMapleAsciiString("DropStone");
        mplew.writeMapleAsciiString("start");
        mplew.write(1);
...
//            mplew.writeInt(1);
    ...
    public static byte[] spawnArrowBlaster(MapleCharacter chr, int x, int y, int a) {
        ...
        mplew.writeShort(SendPacketOpcode.ATTACKOBJ_CREATE.getValue());
        mplew.writeInt(1);
        mplew.writeInt(1);
        mplew.writeInt(chr.getId());
//        mplew.writeInt(0);
    ...
    public static byte[] controlArrowBlaster(int a) {
        ...
        mplew.writeShort(SendPacketOpcode.ATTACKOBJ_SETATTACK.getValue());
        mplew.writeInt(a);
        mplew.writeInt(0);
    ...
    public static byte[] cancelArrowBlaster(int b) {
        ...
        mplew.writeShort(SendPacketOpcode.ATTACKOBJ_REMOVE_BYLIST.getValue());
        mplew.writeInt(1);
        mplew.writeInt(b);
...
//            mplew.write(0);
        ...
        public static byte[] getNPCTalk(int npc, byte msgType, String talk, String endBytes, byte type, byte type2, int diffNPC) {
            ...
            mplew.writeShort(SendPacketOpcode.NPC_TALK.getValue());
            mplew.write(4);
            mplew.writeInt(npc == 0 ? 9131005 : npc);
            mplew.write(0); // Boolean
            mplew.write(msgType);
            mplew.write(type);
//            mplew.write(type2);
