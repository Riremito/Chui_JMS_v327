...
public class ChatHandler {
    ...
    public static void Others(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr == null || !chr.getCanTalk()) {
            c.announce(CWvsContext.broadcastMsg(6, "You have been muted and are therefore unable to talk."));
            return;
        }
        ...
        switch (slea.readByte()) {
            ...
            case 0x0B: //char info
                if (messenger != null) {
                    ...
                    c.announce(CField.messengerCharInfo(character));
                }
                break;
            case 0x0E: //whisper
                if (messenger != null) {
            ...
            case 5: { //搜尋Find
                ...
                if (player != null) {
                    if (!player.isIntern() || c.getPlayer().isIntern() && player.isIntern()) {
                        ...
                        c.announce(CField.getFindReplyWithMap(player.getName(), player.getMap().getId(), mode == 72));//68
                    } else {
