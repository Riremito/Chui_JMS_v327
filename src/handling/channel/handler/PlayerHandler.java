...
//            PlayerHandler.AngelicChange(slea, c, chr);
            if (MapleJob.is天使破壞者(c.getPlayer().getJob())) {
                c.announce(JobPacket.AngelicPacket.ChangeAngelic(type));
...
/*                    if (chr.getBuffedValue(MapleBuffStat.ENERGY_CHARGE) == null && //能量获得
                            ...
                            chr.getBuffedValue(MapleBuffStat.SUMMON) == null && //召喚獸
                            chr.getBuffedValue(MapleBuffStat.RAINING_MINES) == null && //地雷
                            chr.getBuffedValue(MapleBuffStat.TELEPORT_MASTERY) == null && //快速移動精通
                            chr.getBuffedValue(MapleBuffStat.ASURA_IS_ANGER) == null && //修羅
...
//        slea.skip(4);
        ...
        if (chr.isShowInfo()) {
            chr.showMessage(27, "[特殊攻击] 技能: " + SkillFactory.getSkillName(skillId) + "(" + skillId + ") 技能等级: " + skilllevel);
        }
        ...
        chr.getMap().broadcastMessage(chr, CField.EffectPacket.showBuffEffect(false, chr, skillId, SpecialEffectType.LOCAL_SKILL, chr.getLevel(), skilllevel), false);
        chr.getMap().broadcastMessage(chr, CField.showSpecialAttack(chr.getId(), tickCount, pos_x, pos_y, display, skillId, skilllevel, isLeft, speed), chr.getTruePosition());
    }
}
