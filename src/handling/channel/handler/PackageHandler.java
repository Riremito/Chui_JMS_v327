    ...
    public static void handleAction(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (operation) {
            case 1: { // Start Donald, 13 digit AS
                ...
                if (conv == 2) { // Donald
                    c.announce(CField.sendPackageMSG((byte) 10, loadItems(c.getPlayer())));
                }
                break;
            }
            case 3: { // Send Item
                    ...
                    if (accid != -1) {
                        if (accid != c.getAccID()) {
                            ...
                            if (inventId > 0) {
                                ...
                                if (item == null) {
                                    c.announce(CField.sendPackageMSG((byte) 17, null)); // Unsuccessfull
                                    return;
                                }
                                ...
                                if (c.getPlayer().getItemQuantity(item.getItemId(), false) >= amount) {
                                    ...
                                    if (!ii.isDropRestricted(item.getItemId()) && !ii.isAccountShared(item.getItemId())) {
                                        if (addItemToDB(item, amount, mesos, c.getPlayer().getName(), accid, recipientOn)) {
                                            ...
                                            c.announce(CField.sendPackageMSG((byte) 19, null)); // Successfull
                                        } else {
                                            c.announce(CField.sendPackageMSG((byte) 17, null)); // Unsuccessful
                                        }
                                    } else {
                                        c.announce(CField.sendPackageMSG((byte) 17, null)); // Unsuccessfull
                                    }
                            ...
                            //    rClient.announce(CField.sendPackageMSG(Actions.PACKAGE_MSG.getCode()));
                            //}
                        } else {
                            c.announce(CField.sendPackageMSG((byte) 15, null)); // Same acc error
                        }
                    } else {
                        c.announce(CField.sendPackageMSG((byte) 14, null)); // Name does not exist
                    }
                } else {
                    c.announce(CField.sendPackageMSG((byte) 12, null)); // Not enough mesos
                }
                break;
            }
            case 5: { // Recieve Package
                ...
                if (dp.getItem() != null && !MapleInventoryManipulator.checkSpace(c, dp.getItem().getItemId(), dp.getItem().getQuantity(), dp.getItem().getOwner())) {
                    c.announce(CField.sendPackageMSG((byte) 16, null)); // Not enough Space
                    return;
                ...
                c.announce(CField.removeFromPackageList(false, packageid));
                break;
            }
            case 6: { // Remove package
                ...
                c.announce(CField.removeFromPackageList(true, packageid));
                break;
            }
