...
//            cfrom.getClient().announce(GuildPacket.denyGuildInvitation(c.getPlayer().getName()));
    ...
    public static final void JoinGuildCancel(final MapleClient c) {
        ...
        c.announce(GuildPacket.removeGuildJoin(c.getPlayer().getId()));
        c.getPlayer().setGuildId(0);
    }
    ...
    public static final void AddGuildMember(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            chr.getClient().announce(GuildPacket.showGuildInfo(chr));
            final MapleGuild gs = World.Guild.getGuild(c.getPlayer().getGuildId());
            for (byte[] pack : World.Alliance.getAllianceInfo(gs.getAllianceId(), true)) {
                if (pack != null) {
                    chr.getClient().announce(pack);
                }
            }
            chr.saveGuildStatus();
            respawnPlayer(chr);
    ...
    public static final void Guild(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (mode) { //AFTERSHOCK: most are +1
            ...
            case 0x02: // 顯示公會
                ...
                c.announce((byte[]) GuildPacket.getGuildReceipt(guildId));
                break;
            case 0x04: // 檢查公會名稱
                GuildName = slea.readMapleAsciiString();
                if (!isGuildNameAcceptable(GuildName)) {
                    c.announce(GuildPacket.genericGuildMessage((byte) 0x34)); // 公會名稱重複 Version.176 [完成]
                    return;
                }
                c.announce(GuildPacket.createGuildNotice(GuildName));
                break;
            case 0x07: // 邀請
            ...
            case 0x2D: // 公會搜尋
                switch (slea.readByte()) {
                    case 0:// 名字搜尋
                        c.announce(GuildPacket.showSearchGuilds(MapleGuild.searchGuild(slea.readShort(), slea.readByte() == 1, slea.readMapleAsciiString())));
                        break;
                    case 1:// 條件搜尋
                        ...
                        c.announce(GuildPacket.showSearchGuilds(MapleGuild.searchGuild(keyWords)));
                        break;
                }
                break;
            case 0x35: // 建立公會提示 Version.176 [完成]
                ...
                if (create == 1) { // 建立公會成功
                    ...
                    if (guildId == 0) {
                        c.announce(GuildPacket.genericGuildMessage((byte) 0x3E)); // 建立公會異常 Version.176 [完成]
                        return;
                    }
...
//                    c.announce(GuildPacket.newGuildInfo(c.getPlayer()));
                    ...
                    c.announce(GuildPacket.genericGuildMessage((byte) 0x3D)); // Version.176 [完成]
                }
                break;
            default:
