...
public class ItemMakerHandler {
    ...
    public static final void UseBag(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (firstTime) {
            ...
            toUse.setFlag(flag);
            c.announce(InventoryPacket.updateSpecialItemUse(toUse, (byte) 4, toUse.getPosition(), true, chr));
        }
        c.announce(CField.openBag(chr.getExtendedSlots().indexOf(itemId), itemId, firstTime));
        c.announce(CWvsContext.enableActions());
    }
    ...
    public static final void StartHarvest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (lastTime + (5000) > System.currentTimeMillis()) {
            ...
            c.announce(CField.harvestMessage(reactor.getObjectId(), 11)); //ok to harvest, gogo
    ...
    public static void ProfessionInfo(final LittleEndianAccessor slea, final MapleClient c) { //so pointless
        ...
        if (stat.startsWith("9200") || stat.startsWith("9201")) {
            ...
            if ((stat.equalsIgnoreCase("hyper")) || (stat.equalsIgnoreCase("hyper_shaman"))) {
                ...
                c.announce(CWvsContext.updateSpecialStat(stat, array, mode, array < 41, chance));
                return;
            } else if (stat.equalsIgnoreCase("needHyperStatLv")) {
                rate = array < 5 ? (int) Math.pow(2.0, array - 1) : (array - 3) * 5;
                c.announce(CWvsContext.updateSpecialStat(stat, array, mode, true, rate));
                return;
            } else if (stat.equalsIgnoreCase("incHyperStat")) {
                rate = 3 + (array / 10 - 14);
                c.announce(CWvsContext.updateSpecialStat(stat, array, mode, true, rate));
                return;
            } else if (stat.equalsIgnoreCase("honorLeveling")) {
        ...
        c.announce(CWvsContext.updateSpecialStat(stat, array, mode, rate));
    }
    ...
    public static final void CraftEffect(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (profession != null && (c.getPlayer().getProfessionLevel(profession) > 0 || (profession == 92040000 && chr.getMap().getExtractorSize() > 0))) {
            ...
            c.announce(EffectPacket.showCraftingEffect(effect, (byte) chr.getDirection(), time, effect.endsWith("Extract") ? 1 : 0));
    ...
    public static final void UsePot(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        for (int i = 0; i < c.getPlayer().getImps().length; i++) {
            if (c.getPlayer().getImps()[i] == null) {
                ...
                c.announce(CWvsContext.updateImp(c.getPlayer().getImps()[i], ImpFlag.SUMMONED.getValue(), i, false));
    ...
    public static final void ClearPot(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.announce(CWvsContext.updateImp(c.getPlayer().getImps()[index], ImpFlag.REMOVED.getValue(), index, false));
    ...
    public static final void FeedPot(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.announce(CWvsContext.updateImp(c.getPlayer().getImps()[index], mask, index, false));
    }
    ...
    public static final void CurePot(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        c.getPlayer().getImps()[index].setState(1);
        c.announce(CWvsContext.updateImp(c.getPlayer().getImps()[index], ImpFlag.STATE.getValue(), index, false));
