...
public class NPCHandler {
    ...
    public static void NPCAnimation(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (slea.available() > 0) {
            int u4 = slea.readInt();
            Point u5 = slea.readPos();
            Point u6 = slea.readPos();
            List res;
            try {
                res = MovementParse.parseMovement(slea, 8, c.getPlayer());
...
//                chr.getMap().broadcastMessage(chr, EffectPacket.showQuetCompleteEffect(chr.getId()), false);
        ...
        switch (mode) {
            ...
            case 7: {//楓幣存取操作
                ...
                if ((meso > 0L && storageMesos >= meso) || (meso < 0L && playerMesos >= -meso)) {
                    if (meso < 0L && storageMesos - meso > 99999999999L) {//保管
                        meso = -(99999999999L - storageMesos);
                        if (-meso > playerMesos) {
                            return;
                        }
    ...
    public static void NPCMoreTalk(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (lastMsg == NPCTalk.AVATAR.getType() && slea.available() >= 4) {
            slea.readShort();
        }
        final byte action = slea.readByte(); // 00 = end chat, 01 == follow
        final NPCConversationManager cm = NPCScriptManager.getInstance().getCM(c);
        if (cm != null && lastMsg == NPCTalk.JOB_SELECTION.getType()) {
        ...
        cm.setLastMsg((byte) -1);
        if (lastMsg == NPCTalk.UNK_1.getType()) {
            NPCScriptManager.getInstance().action(c, action, lastMsg, -1);
        } else if (lastMsg == NPCTalk.TEXT.getType()) {
            if (action != 0) {
            ...
            if (lastMsg == NPCTalk.NUMBER.getType() && selection == -1) {
                cm.dispose();
                return;//h4x
            }
