    ...
    public static void handlePacket(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        switch (code) {
            case 0: { //咒文的痕跡 強化
                ...
                if (item == null) {
                    c.announce(CWvsContext.Enchant.getEnchantResult(item, 0, sposition, ""));
                    return;
                }
                ...
                if (c.getPlayer().isShowInfo()) {
                    c.getPlayer().showMessage(32, "咒文強化 - 強化道具：" + item + " 選中捲軸:" + scroll.getName() + " 消耗咒文：" + scroll.getCost() + " 成功率：" + scroll.getSuccessRate() + "%  強化結果：" + (success == 1));
                }
                ...
                c.announce(CWvsContext.Enchant.getEnchantResult(item, success, sposition, scroll.getName()));
...
/*                int count_rate = 1;
                ...
                c.announce(CWvsContext.Enchant.getEnchantList(c, item, getEchantScrolls(item), count_rate, ServerConstants.FEVER_TIME));*/
                break;
            }
            case 1: { //星力強化
                slea.skip(4);
                ...
                if (item == null) {
                    c.announce(CWvsContext.Enchant.getStarForceResult(item, item, StarForceResult.失敗.getValue()));
                    return;
                }
                if (EquipStat.EnhanctBuff.EQUIP_MARK.check(item.getEnhanctBuff())) {
                    return;
                ...
                if (c.getPlayer().isShowInfo()) {
                    c.getPlayer().showMessage(32, "星力強化 - 強化道具：" + item + " 當前星級：" + enhance + "星 消耗幣種：" + (maplePoint ? "楓點" : "楓幣") + " 消耗量：" + meso + " 成功機率：" + ((successprop - (mgsuccess ? 0.0 : 100.0)) / 10.0) + "% 小遊戲成功機率加成：" + (mgsuccess ? 10.0 : 0) + "% 損壞機率：" + (destroyprop / 10.0) + "% 失敗(" + (fall ? "下滑" : "維持") + ")機率：" + (fallprop / 10.0) + "% 強化結果：" + result.name());
                }
                ...
                switch (result) {
                    ...
                    case 損壞:
                        ...
                        if (c.getPlayer().isShowInfo()) {
                            c.getPlayer().showMessage(32, "星力強化損毀處理 - 原星級：" + enhance + "損毀后星級：" + item.getEnhance());
                        }
                ...
                c.announce(CWvsContext.Enchant.getStarForceResult(source, item, result.getValue()));
                c.getPlayer().forceReAddItem(item, item.getPosition() >= 0 ? MapleInventoryType.EQUIP : MapleInventoryType.EQUIPPED);
                break;
            }
            case 2: { // 星力強化 繼承
                ...
                c.announce(CWvsContext.Enchant.getInheritance(nEquip.copy(), nEquip.inheritance(equip)));
            ...
            case 0x34: { // 星力強化 放置道具
                ...
                c.announce(CWvsContext.Enchant.getStarForcePreview(stats, meso, successprop, destroyType, fall, item.getFailCount() >= 2));
                break;
            }
            case 0x33:
            case 0x35: { //星力強化成功率加成遊戲
                c.announce(CWvsContext.Enchant.getStarForceMiniGame(code));
                break;
            }
            default: {
