...
public class GameConstants {
    ...
    public static final int JAIL = 180000004, MAX_BUFFSTAT = 13, MAX_MOBSTAT = 13;
...
//        return MapleJob.is末日反抗軍(job) || MapleJob.is龍魔導士(job) || MapleJob.is精靈遊俠(job) || MapleJob.is幻影俠盜(job) || MapleJob.is米哈逸(job) || MapleJob.is夜光(job) || MapleJob.is凱撒(job) || MapleJob.is天使破壞者(job) || MapleJob.is曉の陣(job);
        return !MapleJob.is管理員(job) && !MapleJob.is烈焰巫師(job) && !MapleJob.is暗夜行者(job) && !MapleJob.is幻獸師(job) && !MapleJob.is皮卡啾(job);
    }
